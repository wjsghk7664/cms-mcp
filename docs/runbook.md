...
`project_code` for names such as 한국캐시워크. In CMS app labels, 한국캐시워크 is
usually just `캐시워크`; inventory tools normalize `app_name="한국캐시워크"` to
`캐시워크`. Use `app_name` on inventory tools, or get the app/publisher id from
`cms_report_metadata` and pass it as `publisher_id`/`app_id` for unit or report
queries.
